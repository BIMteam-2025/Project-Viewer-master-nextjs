...
import { Users, User as UserIcon, PlusCircle, Loader2 } from 'lucide-react';
            ...
            <SidebarInset className="flex-col items-center justify-start p-4 sm:p-8">
                <div className="w-full max-w-8xl space-y-8">
                    ...
                    <Dialog onOpenChange={(open) => !open && setSelectedUser(null)}>
                                        ...
                                        <div className="grid gap-4 py-4">
                                            {headers.map(header => (
                                                <div key={header} className="grid grid-cols-4 items-center gap-4">
                                                    <Label htmlFor={`new-user-${header}`} className="text-right">
                                                        {header}
                                                    </Label>
                                                    <Input
                                                        id={`new-user-${header}`}
                        ...
                        {selectedUser && (
                            <DialogContent className="sm:max-w-md sm:min-w-[660px]">
                                <DialogHeader>
                                    ...
                                    {Object.entries(selectedUser).map(([key, value]) => (
                                        key !== 'id' && (
                                            <div key={key} className="grid grid-cols-[1fr_2fr] items-center gap-4">
                                                <span className="text-sm font-medium text-muted-foreground text-left">{key}</span>
